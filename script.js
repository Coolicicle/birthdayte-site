...
Activities: ${state.activities.join(", ") || "None"}`;
  ...
  homeToSummary.addEventListener("click", () => {
    state.activities = [...document.querySelectorAll("#homeActivitiesScreen input:checked")].map(i => i.value);
    updateSummary();
  });

  dayOutToSummary.addEventListener("click", () => {
    state.activities = [...document.querySelectorAll("#dayOutActivitiesScreen input:checked")].map(i => i.value);
    updateSummary();
  });
